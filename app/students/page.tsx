...
async function getStudentListWithStats() {
  ...
  return (
    students?.map((student) => {
      ...
      const allScores =
        student.reports?.flatMap((r: any) => r.report_scores) || [];

      const calc = (cat: string) => {
        const filtered = allScores.filter((s: any) => s.category === cat);
        return filtered.length > 0
          ? (
              filtered.reduce((a: number, b: any) => a + b.score, 0) /
              filtered.length
            ).toFixed(1)
          : "0.0";
      };
            ...
            {recentReports.map((report: any, idx: number) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`text-xs font-black px-3 py-1 rounded-lg ${Number(avg) >= 4 ? "bg-emerald-50 text-emerald-600" : "bg-slate-50 text-slate-600"}`}
                  >
                    {avg}
                  </div>
                </Link>
