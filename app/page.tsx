...
import { Mic, BarChart3, LogOut, ShieldCheck, ArrowUpRight, Check, Sparkles, Users, LineChart, MessageCircle, LayoutDashboard, UserCog, ClipboardList, HeartHandshake, FileText, Percent, Gem, Crown, Award } from "lucide-react";
...
function BentoFeatureCard({
  ...
  return (
    <div className={`group relative overflow-hidden rounded-[1.8rem] border-2 border-slate-100 ${className}`}>
      <img
        src={image}
        alt={title}
      ...
      <div className="relative z-10 h-full flex flex-col justify-end p-6">
        ...
        <h3 className="font-black text-lg leading-snug text-brand-700">{title}</h3>
        <p className="text-sm text-black/75 font-bold leading-relaxed mt-2 max-w-[26rem]">{text}</p>
      </div>
    </div>
  );
}
    ...
    <main className="min-h-screen bg-[#f7f8f3] text-slate-900 overflow-hidden">
      <header className="max-w-6xl mx-auto px-5 sm:px-8 py-6 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-7 text-sm font-black text-slate-500">
          <a href="#tentang" className="hover:text-brand-700 transition-colors">Tentang</a>
      ...
      <section className="max-w-6xl mx-auto px-5 sm:px-8 pt-10 pb-20 lg:pt-20 lg:pb-28 grid lg:grid-cols-[1.02fr_.98fr] gap-14 items-center">
        <div>
          ...
          <div className="mt-9 flex flex-wrap items-center gap-3">
            ...
            <a href="#tentang" className="px-5 py-3.5 rounded-2xl text-slate-600 font-black hover:text-brand-700 transition-colors">Lihat cara kerja</a>
          </div>
      ...
      {/* Tentang */}
      <section id="tentang" className="bg-white border-y-2 py-10 border-slate-100 relative">
        ...
        <img src="/img/landing/cds_home_phone.png" alt="Character Development System" className="lg:w-72 rotate-72 max-lg:hidden h-auto absolute md:top-[20%] md:left-32" />
        
          <div className="relative">

            <p className="text-brand-600 text-xs font-black uppercase tracking-[.2em]">Tentang characterdev</p>
            <h2 className="text-3xl sm:text-4xl font-black tracking-tight mt-3 leading-[1.05]">Character Development System (CDS) untuk guru.</h2>
          </div>
          <div className="grid grid-flow-col max-lg:grid-cols-[1fr_1fr] gap-5 place-items-center">
 <img src="/img/landing/cds_home_phone.png" alt="Character Development System" className="w-48 max-lg:rotate-6 lg:hidden h-auto" />

          <div className="space-y-4 text-slate-600 font-bold leading-relaxed text-base sm:text-lg">
            <p>characterdev adalah aplikasi untuk menanamkan karakter, mental, dan soft skill Qurani ke dalam diri peserta didik, berdasarkan hasil observasi guru terhadap pikiran, perkataan, dan perbuatan mereka sehari-hari.</p>
            <p>Aplikasi ini membantu guru mengobservasi, menganalisis, serta memperoleh gambaran yang komprehensif mengenai pertumbuhan karakter, mental, dan soft skill peserta didik berdasarkan data yang terukur — sekaligus memberikan rekomendasi treatment berupa nasihat, pendekatan, dan strategi pembinaan yang bisa langsung diterapkan.</p>
          </div>
          </div>
      ...
      <section className="bg-[#fbfcf8] relative z-10 border-b-2 border-slate-100">
        <div className="max-w-6xl mx-auto px-5 sm:px-8 py-20">
          ...
          <div className="grid md:grid-cols-3 gap-5">
            ...
            ].map(([Icon, title, text]) => { const ItemIcon = Icon as typeof MessageCircle; return <div key={title as string} className="p-6 rounded-[1.6rem] border-2 border-slate-100 bg-white"><div className="w-11 h-11 rounded-2xl bg-brand-100 text-brand-700 flex items-center justify-center mb-6"><ItemIcon size={20}/></div><h3 className="font-black text-lg">{title as string}</h3><p className="text-sm text-slate-500 font-bold leading-relaxed mt-3">{text as string}</p></div> })}
          </div>
        </div>
      </section>
      ...
      <section id="fitur" className="bg-white border-b-2 border-slate-100">
        <div className="max-w-6xl mx-auto px-5 sm:px-8 py-20">
          <div className="max-w-xl mb-12">
            <p className="text-brand-600 text-xs font-black uppercase tracking-[.2em]">Sisi guru</p>
            <h2 className="text-3xl sm:text-4xl font-black tracking-tight mt-3">Semua yang guru butuhkan untuk mendampingi santri.</h2>
            <p className="mt-4 text-slate-500 font-bold leading-relaxed">Dari input observasi harian sampai rapor perkembangan siap cetak — satu tempat, satu alur.</p>
          </div>
          <div>
      ...
      <section className="bg-[#fbfcf8] border-b-2 border-slate-100">
        <div className="max-w-6xl mx-auto px-5 sm:px-8 py-20">
          <div className="max-w-xl mb-12">
            <p className="text-brand-600 text-xs font-black uppercase tracking-[.2em]">Sisi admin sekolah</p>
            <h2 className="text-3xl sm:text-4xl font-black tracking-tight mt-3">Kendali penuh untuk pimpinan sekolah.</h2>
            <p className="mt-4 text-slate-500 font-bold leading-relaxed">Satu dashboard untuk mengelola guru, santri, dan memantau seluruh proses pembinaan karakter sekolah.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {[
              { image: '/img/landing/admin_overview.png', icon: LayoutDashboard, title: 'Overview', text: 'Ringkasan aktivitas characterdev sekolah secara global: jumlah guru, santri, laporan, dan cakupan ketercapaian.', span: 'sm:col-span-2 lg:col-span-2', height: 'h-72 sm:h-120 lg:h-[26rem]' },
              { image: '/img/landing/admin_kelola_santri.png', icon: Users, title: 'Kelola Santri', text: 'Tambah, lihat, dan tugaskan santri ke guru pembimbing dari satu tempat.', span: 'lg:col-span-1', height: 'h-72 sm:h-120 lg:h-[26rem]' },
              { image: '/img/landing/admin_kelola_guru.png', icon: UserCog, title: 'Kelola Ustadz', text: 'Atur daftar guru dan admin beserta hak aksesnya.', span: '', height: 'h-72 sm:h-120 lg:h-120' },
              { image: '/img/landing/admin_monitor_laporan.png', icon: ClipboardList, title: 'Monitor Laporan', text: 'Pantau progres laporan tiap santri per guru pembimbing.', span: '', height: 'h-72 sm:h-120 lg:h-120' },
              { image: '/img/landing/admin_rencana_penanganan.png', icon: HeartHandshake, title: 'Rencana Penanganan', text: 'Lihat seluruh rencana penanganan yang dibuat guru beserta status tindak lanjutnya.', span: '', height: 'h-72 sm:h-120 lg:h-120' },
      ...
      <section id="harga" className="bg-slate-900 text-white">
        <div className="max-w-6xl mx-auto px-5 sm:px-8 py-20">
          <div className="max-w-xl mb-12">
            <p className="text-brand-300 text-xs font-black uppercase tracking-[.2em]">Investasi pembinaan karakter santri</p>
            <h2 className="text-3xl sm:text-4xl font-black tracking-tight mt-3">Pilih paket sesuai jumlah santri yang dibina.</h2>
          </div>
          <div className="grid md:grid-cols-3 gap-5">
            {[
              [Gem, 'Premium', 'Rp50.000', '/santri/bulan', 'Kuota 1–100 santri termonitor', false],
              [Crown, 'Platinum', 'Rp30.000', '/santri/bulan', 'Kuota 100–300 santri termonitor', true],
              [Award, 'Gold', 'Hubungi Kami', '', 'Khusus monitoring lebih dari 300 santri', false],
            ].map(([Icon, tier, price, unit, desc, highlighted]) => {
              const ItemIcon = Icon as typeof MessageCircle;
              return (
                <div key={tier as string} className={`p-7 rounded-[1.8rem] flex flex-col gap-5 ${highlighted ? 'bg-brand-500 text-white' : 'bg-white/5 border-2 border-white/10'}`}>
                  <div className={`w-11 h-11 rounded-2xl flex items-center justify-center ${highlighted ? 'bg-white/20' : 'bg-white/10 text-brand-300'}`}><ItemIcon size={20}/></div>
                  <div>
                    <p className="text-xs font-black uppercase tracking-widest opacity-70">{tier as string}</p>
                    <p className="text-3xl font-black mt-2 leading-none">{price as string}</p>
                    {(unit as string) && <p className="text-xs font-bold opacity-70 mt-1">{unit as string}</p>}
                  </div>
                  <p className={`text-sm font-bold leading-relaxed ${highlighted ? 'text-white/90' : 'text-slate-300'}`}>{desc as string}</p>
                </div>
              );
            })}
          </div>
          <p className="mt-8 text-xs font-bold text-slate-400 max-w-xl">Semua paket mencakup akses penuh ke fitur guru dan admin — perbedaan hanya pada kuota santri yang termonitor.</p>
        </div>
      </section>

      <footer className="bg-slate-900 text-white border-t border-white/10"><div className="max-w-6xl mx-auto px-5 sm:px-8 py-10 flex flex-col sm:flex-row gap-5 items-start sm:items-center justify-between"><div><p className="font-black text-lg">character<span className="text-brand-400">dev</span></p><p className="text-xs text-slate-400 font-bold mt-1">Mendampingi pertumbuhan, satu cerita pada satu waktu.</p></div><Link href="/login" className="inline-flex items-center gap-2 text-sm font-black text-brand-300 hover:text-white">Masuk ke portal <ArrowUpRight size={15}/></Link></div></footer>
    </main>
  );
}
