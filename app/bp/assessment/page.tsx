...
export default function BpAssessmentPage() {
  ...
  async function finish() {
    ...
    router.push(`/bp/results?id=${encodeURIComponent(studentId)}&name=${encodeURIComponent(studentName)}`);
  }
  ...
  return <div className="flex min-h-screen flex-col bg-paper">
    ...
    <ConfirmModal isOpen={confirmBack} title={isEnglish ? "Leave this conversation?" : "Keluar dari percakapan?"} description={isEnglish ? "Your draft is saved, so you can continue it later." : "Draf Anda tersimpan dan dapat dilanjutkan nanti."} confirmLabel={isEnglish ? "Leave" : "Keluar"} cancelLabel={isEnglish ? "Stay" : "Tetap di sini"} confirmVariant="success" onConfirm={() => router.push("/bp")} onCancel={() => setConfirmBack(false)} />
  </div>;
}
